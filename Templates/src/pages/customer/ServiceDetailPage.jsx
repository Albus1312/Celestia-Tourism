...
import { MapPin, Phone, Info, CheckCircle2, Building } from 'lucide-react';
...
const ServiceDetailPage = () => {
  ...
  const fetchServiceDetail = async () => {
    try {
      setLoading(true);
      const data = await axiosClient.get(`/LocalServices/${id}`);
      setService(data);

      // Log page view
      try {
        await axiosClient.post('/analytics/log-view', {
          localServiceId: parseInt(id)
        });
      } catch (err) {
        ...
        <div className="flex-1 flex items-center justify-center p-6 mt-16">
          <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md text-center w-full transform transition-all">
            ...
            <p className="text-gray-600 mb-8 text-lg">
              Yêu cầu đặt {service.type.toLowerCase()} <strong>{service.name}</strong> của bạn đã được ghi nhận. Chúng tôi sẽ sớm liên hệ xác nhận.
            </p>
            <button 
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-grow pt-20">
        ...
        <div className="relative h-96 w-full">
          <img 
            src={service.imageUrl || 'https://images.unsplash.com/photo-1542314831-c6a4d14b8fc4?q=80&w=1200&auto=format&fit=crop'} 
            alt={service.name} 
          ...
          <div className="absolute bottom-0 w-full p-8 text-white max-w-7xl mx-auto">
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold mb-4 text-shadow-sm">{service.name}</h1>
            <div className="flex items-center text-gray-200 text-lg font-medium">
              <MapPin className="w-5 h-5 mr-2" />
              {service.address} {service.destination ? `- ${service.destination.name}` : ''}
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-900">{service.phone || 'Đang cập nhật'}</p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-900 line-clamp-2">{service.address}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {service.galleryUrls && service.galleryUrls.length > 0 && (
              <ImageGallerySection urls={service.galleryUrls} title="Ảnh nổi bật của Dịch vụ" />
            )}
          </div>
          ...
          <div className="lg:w-1/3">
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 sticky top-28">
              ...
              <form onSubmit={handleBook} className="space-y-5">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Ngày mong muốn</label>
                  <input 
                    type="date" 
                    required
                ...
                <div className="pt-6">
                  <div className="flex justify-between items-center mb-6 text-lg">
                    ...
                    <span className="font-extrabold text-gray-900">
                      {service.price ? (service.price * quantity).toLocaleString('vi-VN') + ' đ' : 'Chờ báo giá'}
                    </span>
                  </div>
                  
                  <button 
                    type="submit" 
                    disabled={isSubmitting || !service.isActive}
                    className={`w-full flex items-center justify-center px-4 py-4 rounded-xl font-bold text-lg transition-colors shadow-md ${
                      !service.isActive 
                        ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                        : 'bg-primary text-white hover:bg-teal-700'
                    }`}
                  >
                    {isSubmitting ? 'Đang xử lý...' : (!service.isActive ? 'Tạm Ngưng Nhận Đặt' : 'Đặt Ngay')}
                  </button>
